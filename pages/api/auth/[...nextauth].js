...
/* Takes in a token & returns a new token with updated `accessToken` & `accessTokenExpires`
 * If an error occurs, returns the old token & an error prop */
...
export default NextAuth({
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      //   Return previous token if the current access token hasnt expired yet
      if (Date.now() < token.accessTokenExpires) {
        return token;
      }

      // if access token expired, try to update it
